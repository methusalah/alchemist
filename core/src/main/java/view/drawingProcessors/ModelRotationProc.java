...
public class ModelRotationProc extends Processor {
	...
	protected void onEntityEachTick(Entity e) {
		...
		double newRoll = r.getxPeriod() != 0? m.getRollFix().getValue()+AngleUtil.FULL/r.getxPeriod()*LogicLoop.TIME_PER_FRAME : m.getRollFix().getValue();
		double newPitch = r.getyPeriod() != 0? m.getPitchFix().getValue()+AngleUtil.FULL/r.getyPeriod()*LogicLoop.TIME_PER_FRAME : m.getPitchFix().getValue();
		double newYaw = r.getzPeriod() != 0? m.getYawFix().getValue()+AngleUtil.FULL/r.getzPeriod()*LogicLoop.TIME_PER_FRAME : m.getYawFix().getValue();
