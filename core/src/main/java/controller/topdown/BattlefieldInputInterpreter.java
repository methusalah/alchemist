...
public class BattlefieldInputInterpreter extends InputInterpreter {
	...
	BattlefieldInputInterpreter(TopdownState controller) {
		super(controller);
		...
		if (!isPressed) {
			...
			switch(name){
				case MULTIPLE_SELECTION:
					...
					break;
				case SELECT:
					((TopdownState) ctrl).startSelectionZone();
					break;
			}
		}
	}
	...
	private Point2D getSpatialCoord() {
		return ctrl.spatialSelector.getCoord((((TopdownState) ctrl).view.getRootNode()));
	}
}
