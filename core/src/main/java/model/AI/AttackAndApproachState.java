...
public class AttackAndApproachState extends EntityStateActor {
	private final EntityId target;
	
	public AttackAndApproachState(EntityData entityData, EntityId eid, EntityId target) {
	...
	protected void execute() {
		...
		entityData.setComponent(eid, new PlanarNeededThrust(vector.getNormalized()));
		if(vector.getLength() > 12 || vector.getLength() < 8)
			endExecution();
