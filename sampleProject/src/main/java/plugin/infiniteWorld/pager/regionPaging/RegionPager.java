...
public class RegionPager extends BuilderState {
	...
	public RegionPager() {
		...
		RendererPlatform.getMainSceneNode().attachChild(worldNode);
	}
	...
	private RegionCreator instanciateCreator(RegionId id){
		return new RegionCreator(id,
				loader,
				RendererPlatform.getStateManager().getState(DataState.class).getEntityData(),
				RendererPlatform.getStateManager().getState(WorldProc.class).getWorldEntity(),
				(region, drawer) -> {
